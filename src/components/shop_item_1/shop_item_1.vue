  ...
  <div id="shop_item_1">
    <div class="wrapper">
        ...
        <ul :class="{'no_pager': !pager_show}">
            <li :class="{'no_margin_right': index>0 && (index+1)%4 === 0}" v-for="(item,index) in user_goods.length" :key = "index">
                ...
                <div v-if="!selected" class="btn">
                        <span class="alert" ><a href="#"><i class="el-icon-document"></i>修改</a></span>
                        <span class="under" @click.stop.prevent="Up_sell(user_goods[index].id)"><a href="#"><i class="el-icon-delete"></i>下架</a></span>
                        <span class="selled" @click.stop.prevent="Selled(user_goods[index].id)"><a href="#"><i class="el-icon-check"></i>售出</a></span>
                </div>
                <div v-else class="btn1" @click.stop.prevent="Delete(user_goods[index].id)">
    ...
    <transition name="fade"> 
        <div class="confirm" v-if="confirm_show">
                <span class="wrapper"  v-if="confirm_show">
                    <div class="title">{{confirm_content[index]}}</div>
                    <div class="content2">
                        <div class="btn">
                            <span @click.stop.prevent="confirm_btn"><a href="#">确定</a></span><span @click.stop.prevent="confirm_show = false;"><a href="#">取消</a></span>
                        </div>
                    </div>
                </span>
        </div>
...
export default {
    ...
    methods: {
        ...
        confirm_btn() {
            ...
            if (this.index === 0) {
                ...
                option.url = '/unusedgoods/user_goods_delete?id=' + this.key
                option.withCredentials = true
                this.$http(option).then(function (successData) {
                    if (successData.data.id !== -1) {
                        that.prompt_str = '删除成功'
                    }
                ...
                option.method = 'get'
                option.headers = {"Content-Type": "application/json;charset=utf-8"}    
                option.url = '/unusedgoods/user_goods_up?id=' + this.key
                option.withCredentials = true
                this.$http(option).then(function (successData) {
                    // console.log(successData.data.value)
                    if (successData.data.id !== -1) {
                        if (that.index === 1) {
                            that.prompt_str = '售出成功'
                        }
                        else that.prompt_str = '下架成功'
                        that.prompt_show = true
                    }
                    else {
                        // that.prompt_str = '服务器君不玩了！'
        ...
        sell() {
            ...
            option.method = 'get'
            option.headers = {"Content-Type": "application/json;charset=utf-8"}    
            option.url = '/unusedgoods/user_onsell_goods?start=0'
            option.withCredentials = true
            this.$http(option).then(function (successData) {
            that.$nextTick(() => {
        ...
        up_sell() {
            ...
            option.method = 'get'
            option.headers = {"Content-Type": "application/json;charset=utf-8"}    
            option.url = '/unusedgoods/user_upsell_goods?start=0'
            option.withCredentials = true
            this.$http(option).then(function (successData) {
            that.$nextTick(() => {
                that.user_goods = successData.data.list
        ...
        pager_change(val) {
            ...
            option.headers = {"Content-Type": "application/json;charset=utf-8"}    
            if (that.fun_index === 1) {
                option.url = '/unusedgoods/user_onsell_goods?start=' + val
            }
            else option.url = '/unusedgoods/user_upsell_goods?start=' + val
            option.withCredentials = true
    ...
    data() {
        return {
            confirm_show: false,
            confirm_content: ['是否确定删除！','是否确定售出！','是否确定下架！'],
            index: 1,
            fun_index: 1,
            is_confirm: false,
            selected: false,
